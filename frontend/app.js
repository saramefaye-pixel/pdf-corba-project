...
async function mergePDFs() {
    ...
    if (files.length < 2) return showResult('mergeResult', '⚠️ Sélectionnez au moins 2 fichiers PDF.', 'error');
    ...
    try {
        ...
        showResult('mergeResult', '✅ Fusion réussie ! Le PDF a été téléchargé.', 'success');
    } catch(e) { showResult('mergeResult', '❌ Erreur : ' + e.message, 'error'); }
}

async function splitPDF() {
    const file = document.getElementById('splitFile').files[0];
    if (!file) return showResult('splitResult', '⚠️ Sélectionnez un fichier PDF.', 'error');
    const form = new FormData();
    ...
    try {
        ...
        showResult('splitResult', `✅ PDF découpé en ${data.length} page(s) avec succès.`, 'success');
...
async function extractPages() {
    ...
    if (!file) return showResult('extractResult', '⚠️ Sélectionnez un fichier PDF.', 'error');
    if (!pages) return showResult('extractResult', '⚠️ Indiquez les numéros de pages (ex: 1,3,5).', 'error');
    const form = new FormData();
    form.append('file', file);
    ...
    try {
        ...
        showResult('extractResult', '✅ Pages extraites ! Le PDF a été téléchargé.', 'success');
...
async function deletePage() {
    ...
    if (!file) return showResult('deleteResult', '⚠️ Sélectionnez un fichier PDF.', 'error');
    if (index === '') return showResult('deleteResult', '⚠️ Indiquez le numéro de page à supprimer.', 'error');
    const form = new FormData();
    form.append('file', file);
    ...
    try {
        ...
        showResult('deleteResult', '✅ Page supprimée ! Le PDF modifié a été téléchargé.', 'success');
...
async function addPassword() {
    ...
    if (!file) return showResult('passResult', '⚠️ Sélectionnez un fichier PDF.', 'error');
    if (!pass) return showResult('passResult', '⚠️ Saisissez un mot de passe.', 'error');
    if (pass.length < 4) return showResult('passResult', '⚠️ Mot de passe trop court (minimum 4 caractères).', 'error');
    const form = new FormData();
    form.append('file', file);
    ...
    try {
        ...
        showResult('passResult', '✅ PDF protégé avec succès ! Téléchargement effectué.', 'success');
    } catch(e) { showResult('passResult', '❌ Erreur : ' + e.message, 'error'); }
}

async function toImages() {
    const file = document.getElementById('imgFile').files[0];
    if (!file) return showResult('imgResult', '⚠️ Sélectionnez un fichier PDF.', 'error');
    const form = new FormData();
    form.append('file', file);
    try {
        showResult('imgResult', '⏳ Conversion en cours... (peut prendre quelques secondes)', 'info');
...
async function extractText() {
    ...
    if (!file) return showResult('textResult', '⚠️ Sélectionnez un fichier PDF.', 'error');
    const form = new FormData();
    form.append('file', file);
    try {
        ...
        if (!text || text.trim() === '') {
            showResult('textResult', '⚠️ Aucun texte trouvé dans ce PDF.', 'error');
        } else {
...
async function createPDF() {
    ...
    if (!content) return showResult('createResult', '⚠️ Saisissez du contenu pour créer le PDF.', 'error');
    if (content.length < 3) return showResult('createResult', '⚠️ Contenu trop court.', 'error');
    const form = new FormData();
